<div class="container py-4">
        ...
        <div class="card-body p-4">
          <div class="d-flex justify-content-between align-items-start mb-2">
            ...
            <div>
              <?php if (Session::get('userId')): ?>
                <button class="btn btn-sm btn-outline-danger" onclick="toggleFavorite(<?= $product['productId'] ?>)">
                  <i class="far fa-heart"></i>
                </button>
              <?php endif; ?>
              <button class="btn btn-sm btn-outline-secondary" onclick="shareProduct()">
                <i class="fas fa-share-alt"></i>
              </button>
            </div>
          </div>
          
          <h4 class="font-weight-bold mb-2"><?= htmlspecialchars($product['listingTitle']) ?></h4>
          ...
          <div class="mb-3">
            ...
            <?php if ($product['isSold']): ?>
              <span class="badge badge-danger"><?= Session::get("lang") == "tc" ? "已售出" : "Sold" ?></span>
            <?php endif; ?>
          </div>
          
          <div class="seller-stats mb-3">
            <span><i class="far fa-clock mr-1"></i><?= Lang::$lang['postedOn'] ?> <?= date('Y-m-d', strtotime($product['createdDate'])) ?></span>
          ...
          <?php if ($product['seller']): ?>
            <div class="d-flex align-items-center mb-3">
              <div class="mr-3">
                ...
                <?php else: ?>
                  <div class="avatar avatar-lg bg-warning d-flex align-items-center justify-content-center text-white font-weight-bold" style="font-size:1.5rem;">
                    <?= strtoupper(substr($product['seller']['firstName'], 0, 1)) ?>
                  </div>
                <?php endif; ?>
              </div>
              <div class="flex-grow-1">
                <a href="<?= Url::getDomain() ?>profile/<?= $product['seller']['seo'] ?>/" class="font-weight-bold text-dark">
                  <?= htmlspecialchars($product['seller']['firstName'] . ' ' . $product['seller']['lastName']) ?>
                </a>
                <div class="small text-muted">
                  <?php if ($product['seller']['responseRate'] > 0): ?>
                    <span class="mr-2"><i class="fas fa-reply mr-1"></i><?= $product['seller']['responseRate'] ?>% <?= Session::get("lang") == "tc" ? "回覆率" : "response rate" ?></span>
                  <?php endif; ?>
                  <?php if ($product['seller']['responseTime']): ?>
                    <span><i class="fas fa-clock mr-1"></i><?= $product['seller']['responseTime'] ?></span>
                  <?php endif; ?>
                </div>
              </div>
            </div>
            ...
            <?php if (Session::get('userId')): ?>
              <?php if (Session::get('userId') != $product['seller']['userId']): ?>
                <a href="<?= Url::getDomain() ?>inbox/<?= $product['seller']['userId'] ?>/" class="btn btn-yellow btn-block mb-2">
                ...
                <div class="d-flex">
                  ...
                  <button class="btn btn-sm btn-outline-danger" onclick="reportProduct(<?= $product['productId'] ?>)">
                    <i class="fas fa-flag"></i>
                  </button>
                  <button class="btn btn-sm btn-outline-secondary" onclick="toggleBlock(<?= $product['seller']['userId'] ?>)">
  ...
  <div class="row">
    <div class="col-lg-7">
      <div class="card border-0 shadow-sm">
        <div class="card-body p-4">
          <h5 class="font-weight-bold mb-3"><?= Lang::$lang['description'] ?></h5>
          <p class="text-muted" style="white-space: pre-wrap;"><?= nl2br(htmlspecialchars($product['description'])) ?></p>
          
          <?php if ($product['keyword']): ?>
            <hr>
            <h6 class="font-weight-bold mb-2"><?= Lang::$lang['keywords'] ?></h6>
            <div>
      ...
      <div class="card border-0 shadow-sm mt-3">
        <div class="card-body p-4">
          <h5 class="font-weight-bold mb-3"><?= Session::get("lang") == "tc" ? "類似商品" : "Similar Products" ?></h5>
          <div class="row">
            <?php foreach ($similarProducts as $sp): ?>
              <div class="col-lg-3 col-md-4 col-6 mb-3">
                <div class="card product-card h-100">
                  <a href="<?= Url::getDomain() ?>product/<?= $sp['refId'] ?>/">
                    <img src="<?= $sp['image'] ?: Url::getDomain() . 'images/test.jpg' ?>" class="card-img-top" alt="<?= htmlspecialchars($sp['listingTitle']) ?>">
                  </a>
    ...
    <div class="col-lg-5">
      ...
      <div class="card border-0 shadow-sm">
        <div class="card-body p-4">
          <h5 class="font-weight-bold mb-3"><?= Session::get("lang") == "tc" ? "賣家其他商品" : "More from this seller" ?></h5>
          <div class="row">
