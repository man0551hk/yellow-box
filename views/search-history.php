<div class="container py-4">
  <div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="font-weight-bold mb-0"><?= Lang::$lang["searchHistory"] ?></h4>
    ...
    <div class="list-group">
      <?php foreach ($history as $item): ?>
        <a href="<?= Url::getDomain() ?>search/?keyword=<?= urlencode($item['keyword']) ?>" class="list-group-item list-group-item-action d-flex align-items-center">
          ...
          <span class="flex-grow-1"><?= htmlspecialchars($item['keyword']) ?></span>
          <small class="text-muted"><?= date('Y-m-d H:i', strtotime($item['createdDate'])) ?></small>
        </a>
      <?php endforeach; ?>
    </div>
  <?php else: ?>
